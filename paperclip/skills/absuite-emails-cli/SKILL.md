...
- **No atomic partial update in the CLI.** For a safe single-field change, use the `absuite-emails` REST skill's `PATCH` (JSON Patch), where that service exposes one.
- Use **`count <entity>`** (a dedicated operation) to size a collection. OData filtering/paging (`$filter`, `$top`, ...) is REST-only — the CLI does not expose it; use `absuite-emails` for filtered queries or a filtered count.
